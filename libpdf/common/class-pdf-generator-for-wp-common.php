...
class Pdf_Generator_For_Wp_Common
{
  ...
  public function pgfw_generate_pdf($prod_id)
  {
    ...
    $return               = $this->pgfw_generate_pdf_from_library($prod_id, $pgfw_generate_mode);
    print_r($return);die;
  ...
  public function pgfw_generate_pdf_from_library($prod_id, $pgfw_generate_mode, $mode = '', $email = '', $template = '', $template_name = '')
  {
    ...
    $pgfw_generate_mode = 'upload_on_server_and_mail';
    //$pgfw_generate_mode = 'preview';
    ...
    if ('previewj' === $pgfw_generate_mode) {
    ...
    if ('download_locally' === $pgfw_generate_mode) {
      ...
      if ($result) {
        ...
        $local_file = $path; //path to a local file on your server
...
// Upload the file
  ...
  //      $payload .= 'Content-Type: image/jpeg' . "\r\n";
        ...
        $payload .= '--' . $boundary . '--';
        $response = wp_remote_post( LUTINXURL . "PluginApi/Uploadpdf/",
          array(
            'headers'    => $headers,
            'body'       => $payload,
          )
        );
          //print_r(file_get_contents( $local_file ));die;
      ...
      //return 'true';
      //echo admin_url( '/admin.php?page=userdashboard' );die;
      wp_safe_redirect(admin_url('/admin.php?page=userdashboard'));
      //@file_put_contents($path, $output); // phpcs:ignore
      //die('dd');
      //print_r($upload_basedir);die;
